...
# Randomized cut
set.seed(12345)
CompanyData_ups <- CompanyData_ups[sample(nrow(CompanyData_norm)),]
...
```{r, cache = TRUE}
m_ann <- neuralnet(formula = Bankrupt. ~ ., data = CompanyData_ups, stepmax = 1e8)
```
```{r}
plot(m_ann)
```
...
table(m_ann_binary)
table(CompanyData_ups$Bankrupt.)

library(caret)
confusionMatrix(as.factor(m_ann_binary), as.factor(CompanyData_ups$Bankrupt.), positive = "1")
...
simplemodel <- glm(Bankrupt. ~ ., data = CompanyData_ups, family = binomial(link = "logit"), maxit = 1e8)
#summary(simplemodel)
...
```{r}
library(caret)
confusionMatrix(as.factor(pred_cat), as.factor(CompanyData_ups), positive = "1")
```
## SVM
### Run model
```{r}
...
```
```{r}
m_svm <- ksvm(as.factor(Bankrupt.) ~ ., data = CompanyData_ups, kernel = "rbfdot")
...
agreement_sum <- m_svm_prediction == CompanyData_ups$Bankrupt.

prop.table(table(agreement_sum))

confusionMatrix(as.factor(m_svm_prediction), as.factor(CompanyData_ups$Bankrupt.), positive = "1")
...
```{r}
library(C50)
CompanyData_ups$Bankrupt. <- as.factor(CompanyData_ups$Bankrupt.)
decision_tree_model <- C5.0(Bankrupt. ~ ., data = CompanyData_ups)
...
library(gmodels)
library(caret)
confusionMatrix(as.factor(decision_tree_model_result), as.factor(CompanyData_ups$Bankrupt.), positive = '1')
```
...
library(class)
knn_x <- CompanyData_ups[ ,-which(colnames(CompanyData_ups) == "Bankrupt.")]
knn_label <- CompanyData_ups[ , which(colnames(CompanyData_ups) == "Bankrupt.")]
...
confusionMatrix(as.factor(knn_model), as.factor(CompanyData_ups$Bankrupt.), positive = '1')
```
