...
set.seed(300)
decision_tree_model <- train(Class ~ ., data = CompanyData_ups, method = "C5.0",
           metric = "Kappa",
           trControl = ctrl,
           tuneGrid = grid)
...
## Testing
```{r}
AMC_data <- read.csv("AMC_data.csv", stringsAsFactors = TRUE)

AMC_data_norm <- as.data.frame(lapply(AMC_data, normalize))

AMC_data_norm <- AMC_data_norm[1,]

head(AMC_data_norm)


decision_tree_model_AMCresult <- predict(decision_tree_model, AMC_data_norm)
head(decision_tree_model_AMCresult)
```
